...
def decode_record(block: bytes, index: int) -> dict[str, object]:
    ...
    elif subsystem == SUB_DRIVER and event == EV_DRIVER_STATE:
        family = value0 & 0xFFFFFF00
        if family == 0x49324300:
            stage = value0 & 0xFF
            result["diagnostic"] = {
                "kind": "i2c-state",
                "stage": stage,
                "stage_name": {
                    ...
                    2: "no-designware-controller",
                    3: "controller-configured",
                    4: "hid-ready",
                    5: "hid-probe-failed",
                    6: "input-live",
                    7: "precision-mode-failed",
                    8: "precision-mode-selected",
                }.get(stage, f"stage-{stage}"),
                "detail0": value1,
                "detail1": value2,
            }
        elif family == 0x49324400:
            result["diagnostic"] = {
                "kind": "i2c-report-descriptor-header",
                "length": value1,
                "register": value2,
            }
        elif family == 0x49324500:
            data = struct.pack("<II", value1 & 0xFFFFFFFF, value2 & 0xFFFFFFFF)
            result["diagnostic"] = {
                "kind": "i2c-report-descriptor-chunk",
                "chunk": value0 & 0xFF,
                "data_hex": data.hex(),
            }
        elif family == 0x49325000:
            data = struct.pack("<II", value1 & 0xFFFFFFFF, value2 & 0xFFFFFFFF)
            low = value0 & 0xFF
            result["diagnostic"] = {
        ...
        for record in boot["records"]:
            ...
            if diagnostic:
                ...
                if kind == "storage-refused":
                    ...
                    suffix += (
                        f" i2c={diagnostic['stage_name']}"
                        f" detail={diagnostic['detail0']:#x},{diagnostic['detail1']:#x}"
                    )
                elif kind == "i2c-report-descriptor-header":
                    suffix += (
                        f" i2c-report-descriptor length={diagnostic['length']}"
                        f" register={diagnostic['register']:#x}"
                    )
                elif kind == "i2c-report-descriptor-chunk":
                    suffix += (
                        f" i2c-report-descriptor chunk={diagnostic['chunk']}"
                        f" data={diagnostic['data_hex']}"
                    )
                elif kind == "i2c-touch-report-chunk":
                    suffix += (
                        f" i2c-touch-report report={diagnostic['report']}"
                        f" chunk={diagnostic['chunk']} data={diagnostic['data_hex']}"
                    )
            lines.append(
