...
class ZllogTests(unittest.TestCase):
    ...
    def test_i2c_descriptor_and_touch_report_chunks_decode(self) -> None:
        mode = zllog.decode_record(
            zllog.encode_record(
                sequence=4, tsc=254, cpu=0, severity=0,
                subsystem=zllog.SUB_DRIVER, event=zllog.EV_DRIVER_STATE,
                value0=0x49324308, value1=4, value2=3,
            ), 3,
        )
        self.assertEqual(mode["diagnostic"], {
            "kind": "i2c-state",
            "stage": 8,
            "stage_name": "precision-mode-selected",
            "detail0": 4,
            "detail1": 3,
        })

        descriptor = zllog.decode_record(
            zllog.encode_record(
                sequence=5, tsc=255, cpu=0, severity=0,
                subsystem=zllog.SUB_DRIVER, event=zllog.EV_DRIVER_STATE,
                value0=0x49324503,
                value1=0x04030201, value2=0x08070605,
            ), 4,
        )
        self.assertEqual(descriptor["diagnostic"], {
            "kind": "i2c-report-descriptor-chunk",
            "chunk": 3,
            "data_hex": "0102030405060708",
        })

        report = zllog.decode_record(
            zllog.encode_record(
                sequence=6, tsc=256, cpu=0, severity=0,
                subsystem=zllog.SUB_DRIVER, event=zllog.EV_DRIVER_STATE,
                value0=0x49325021,
                value1=0x0D0C0B0A, value2=0x11100F0E,
            ), 5,
        )
        self.assertEqual(report["diagnostic"], {
            "kind": "i2c-touch-report-chunk",
            "report": 2,
            "chunk": 1,
            "data_hex": "0a0b0c0d0e0f1011",
        })
    ...
    def test_typed_audit_lifecycle_and_signed_operation_result_decode(self) -> None:
        lifecycle = zllog.decode_record(
            zllog.encode_record(
                sequence=7, tsc=280, cpu=0, severity=0, subsystem=12,
                event=zllog.EV_LIFECYCLE,
                value0=7 | (1 << 32), value1=1 << 32, value2=4096,
            ), 6,
        )
        self.assertEqual(lifecycle["diagnostic"], {
            "kind": "lifecycle", "object_kind": 1,
            "object_kind_name": "process", "object_id": 7,
            "action": 1, "action_name": "start", "parent_id": 0,
            "detail": 4096,
        })
        begin = zllog.decode_record(
            zllog.encode_record(
                sequence=8, tsc=281, cpu=0, severity=0, subsystem=10,
                event=zllog.EV_OPERATION_BEGIN, value0=99,
                value1=7 | (1 << 32), value2=3 | (0x2006 << 32),
            ), 7,
        )
        self.assertEqual(begin["diagnostic"]["operation_name"], "file-read")
        self.assertEqual(begin["diagnostic"]["actor_id"], 7)
        result = zllog.decode_record(
            zllog.encode_record(
                sequence=9, tsc=282, cpu=0, severity=1, subsystem=10,
                event=zllog.EV_OPERATION_RESULT, value0=99,
                value1=(1 << 64) - 5, value2=0x2006 | (5 << 32),
                payload=struct.pack("<I", 128),
            ), 8,
        )
        self.assertEqual(result["diagnostic"], {
            "kind": "operation-result", "operation_id": 99,
            "operation": 0x2006, "operation_name": "file-read",
            "result": -5, "error": 5, "detail": 128,
        })
        correlated = zllog.audit_report({
            "source": "unit", "warnings": [],
            "boots": [{"boot_id": 1, "slot_index": 0, "state": "writing",
                       "records": [lifecycle, begin, result]}],
        })
        audited = correlated["boots"][0]
        self.assertEqual(len(audited["lifecycle"]), 1)
        self.assertEqual(len(audited["transactions"]), 1)
        self.assertEqual(audited["transactions"][0]["result"], -5)
        self.assertEqual(audited["incomplete_operations"], [])
        self.assertEqual(audited["unmatched_results"], [])
        self.assertIn("file-read", zllog._human_audit(correlated))
