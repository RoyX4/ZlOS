...
ROOT = HERE.parent
JSON_OUTPUT = HERE / "provenance-viewer.json"
HTML_OUTPUT = HERE / "docs" / "provenance-viewer.html"
INPUTS = (
    "build-identity.json",
...
def health_rows(docs: dict[str, dict]) -> list[dict]:
    tests = docs["hosttest/test-run-receipt.json"]["counts"]
...
def validate_model(value: dict, docs: dict[str, dict]) -> None:
    ...
    expected_inputs = [{
        "path": "kernel/" + path, "sha256": sha256(HERE / path),
        "schema": docs[path].get("schema"),
    } for path in INPUTS]
