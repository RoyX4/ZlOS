#!/usr/bin/env python3
"""probe-rail.py - do the REGISTER rows launch what they name?

A rail that launches nothing looks exactly like a rail that launches something,
and this is not hypothetical here. The register was re-pointed at the
prototype's fourteen apps and `rail_launch` had no path for nine of them: it
fell through to `rail_cmd(slot)`, a SECOND table keyed by position which still
said "slot 4 is the editor, slot 5 is paint". Under the new register slot 4 is
the kernel log. Clicking it would have opened the editor, and every screenshot
of the rail would have looked perfect.

probe-app-routes.py used to ask this question and has not run since the dock
became the rail - it parses `dock_app` out of kernel.zl and that function was
deleted with the dock at 07fbf1d. It is wired into gates/land-gate.sh, which is
not what runs on push, so nothing said so. This probe is the narrow replacement
for the part that matters: geometry derived from the rail's own constants, a
real click, and a pixel comparison.

Absolute pointer events, because zlOS prefers the usb-tablet when present and
...
def main():
    ...
    try:
        ...
        # 05 KERNEL LOG - index 4. The row that rail_cmd used to send to the
        # editor, so it is the one that proves the fall-through is gone.
        tx, ty = geo["rows"][4]
        at(qmp, tx, ty)
    ...
    print("probe-rail: PASS - row 05 launches the app it names, from the "
