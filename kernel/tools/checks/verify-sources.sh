...
selftest_recovery() {
    ...
    cp "$SCRIPT_PATH" "$fixture/tools/checks/verify-sources.sh"
    chmod +x "$fixture/tools/checks/verify-sources.sh"
    ...
    printf '%s\n' '99999999' > "$fixture/.verify-sources-state/pid"
    set +e
    (cd "$fixture" && ./tools/checks/verify-sources.sh --recover-only) >/dev/null 2>&1
    rc=$?
    set -e
    [ "$rc" -eq 2 ]
...
if ! python3 ./tools/generators/gen-build-identity.py --write >/dev/null; then
...
build_and_look "raw disk    " ./tools/images/mkdisk.sh     kernel_raw.bin
...
for pair in "./build.sh kernel.elf" "./build64.sh kernel64.elf" \
            "./buildefi.sh ZLOS.EFI" "./tools/images/mkdisk.sh kernel_raw.bin"; do
    set -- $pair
    "$1" >/dev/null 2>&1
