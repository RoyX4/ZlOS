...
def sources():
    source_dirs = [os.path.join(KERNEL_ROOT, "src"),
                   os.path.join(KERNEL_ROOT, "apps"),
                   KERNEL_ROOT]
    root = os.path.join(source_dirs[0], "kernel.zl")
    pending = [root]
    result = {}
    while pending:
        path = pending.pop()
        ...
        for module in IMPORT.findall(text):
            candidates = [os.path.join(directory, module + ".zl")
                          for directory in source_dirs]
            imported = next((candidate for candidate in candidates
                             if os.path.isfile(candidate)), None)
            if imported is None:
                searched = ", ".join(candidates)
                raise SystemExit(f"imported zl source is missing: {module} "
                                 f"(searched {searched})")
