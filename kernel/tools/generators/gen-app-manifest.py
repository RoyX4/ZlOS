...
def embedded(serialized_manifest):
    """The receipt the kernel prints about itself.

    THE COUNT IS READ OUT OF THE MANIFEST, not written here. It used to be the
    literal `entries=64` sitting beside a digest computed from the same string -
    which is how the whole family of stale counts started: the kernel printed a
    number nobody derived, so every gate that checked the kernel had to write
    the same number down, and when apps were added this line was bumped and the
    six gates were not. See docs/evidence/one-fact-many-copies-2026-08-27.md.
    """
    digest = hashlib.sha256(serialized_manifest.encode("utf-8")).hexdigest()
    n_entries = len(json.loads(serialized_manifest)["entries"])
    return (
        "# Generated by gen-app-manifest.py. Do not edit.\n"
        "# This receipt is compiled into every kernel build route.\n"
        "fn app_manifest_report() {\n"
        f'    print("  app-manifest: schema=1 entries={n_entries} sha256={digest}")\n'
        "    return 0\n"
        "}\n"
    )
...
def main(argv):
    ...
    try:
        ...
        return 1
    print("app-manifest: PASS: 63 named implementations + 1 catalogue surface, 24 games")
    return 0
