...
arrays. The output covers 63 current named implementations plus the separate
...
def validate_manifest(manifest, dense, catalog_id):
    ...
    # 64 = 63 named implementations plus the All Applications catalogue.
    # It was 62 until kernel.zl gained 10 SYSTEM (71) and 14 TYPE (72), the two
    # panes the prototype's settings scene shows beside 11 SETTINGS. The number
    # is asserted rather than derived on purpose - it is the guard that makes an
    # ACCIDENTAL app visible - so moving it is part of adding one deliberately.
    if len(entries) != 64:
        fail(f"manifest has {len(entries)} entries, expected 64")
    if len(set(ids)) != len(ids):
        fail("manifest has duplicate app ids")
    if len(set(names)) != len(names):
        fail("manifest has duplicate app names")
    if manifest.get("named_implementation_count") != 63:
        fail("named implementation count is not 63")
...
def manifest_marker(manifest, serialized_manifest):
    schema = manifest["schema"]
    match = re.fullmatch(r"zlos\.application-identity-manifest\.v(\d+)", schema)
    if match is None:
        fail(f"manifest schema cannot be embedded: {schema!r}")
    digest = hashlib.sha256(serialized_manifest.encode("utf-8")).hexdigest()
    return f"app-manifest: schema={match.group(1)} entries={len(manifest['entries'])} sha256={digest}"


def embedded(manifest, serialized_manifest):
    return (
        "# Generated by gen-app-manifest.py. Do not edit.\n"
        "# This receipt is compiled into every kernel build route.\n"
        "fn app_manifest_report() {\n"
        f'    print("  {manifest_marker(manifest, serialized_manifest)}")\n'
        "    return 0\n"
        "}\n"
    )
...
def main(argv):
    ...
    if sum((args.write, args.check, args.marker)) != 1:
        parser.error("choose exactly one of --write, --check, or --marker")
    try:
        manifest = build_manifest()
        expected = serialized(manifest)
        expected_embed = embedded(manifest, expected)
        if args.marker:
            print(manifest_marker(manifest, expected))
            return 0
        if args.write:
        ...
        return 1
    game_count = sum(entry["kind"] == "game" for entry in manifest["entries"])
    print(
        "app-manifest: PASS: "
        f"{manifest['named_implementation_count']} named implementations + "
        f"{manifest['catalogue_surface_count']} catalogue surface, {game_count} games"
    )
    return 0
