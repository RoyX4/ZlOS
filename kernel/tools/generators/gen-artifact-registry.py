...
OUTPUT = os.path.join(METADATA, "artifact-registry.json")
...
def artifact_path(name):
    if name == "boot-media-ids.json":
        return os.path.join(METADATA, name)
...
def format_checks():
    values = {name: open(artifact_path(name), "rb").read(4096)
    ...
    try:
        subprocess.run(
            [sys.executable, ELF_PERMISSION_CHECKER,
             str(artifact_path("kernel.elf")), str(artifact_path("kernel64.elf")),
             str(artifact_path("kernel_raw.elf"))],
            cwd=KERNEL_ROOT, check=True, stdout=subprocess.DEVNULL,
    ...
    with open(artifact_path("zlOS-usb.img"), "rb") as handle:
        handle.seek(512)
...
def extract_relations():
    with tempfile.TemporaryDirectory(prefix="zlos-artifact-registry-") as root:
        ...
        subprocess.run([
            "xorriso", "-osirrox", "on", "-indev", artifact_path("zlOS.iso"),
            "-extract", "/boot/kernel.elf", iso_kernel,
        ], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if sha256(iso_kernel) != sha256(artifact_path("kernel.elf")):
            fail("zlOS.iso does not contain the current kernel.elf")

        iso64_kernel = os.path.join(root, "iso-kernel64.elf")
        subprocess.run([
            "xorriso", "-osirrox", "on", "-indev", artifact_path("zlOS64.iso"),
            "-extract", "/boot/kernel64.elf", iso64_kernel,
        ], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if sha256(iso64_kernel) != sha256(artifact_path("kernel64.elf")):
            fail("zlOS64.iso does not contain the current kernel64.elf")

        usb_efi = os.path.join(root, "usb-BOOTX64.EFI")
        subprocess.run([
            "mcopy", "-n", "-i", artifact_path("zlOS-usb.img") + "@@1M",
            "::/EFI/BOOT/BOOTX64.EFI", usb_efi,
        ], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        if sha256(usb_efi) != sha256(artifact_path("BOOTX64.EFI")):
            fail("zlOS-usb.img does not contain the current BOOTX64.EFI")

        raw_binary = os.path.join(root, "kernel_raw.bin")
        subprocess.run([
            "objcopy", "-O", "binary", artifact_path("kernel_raw.elf"), raw_binary,
        ], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        binary = open(raw_binary, "rb").read()
        disk = open(artifact_path("zlOS.img"), "rb").read()
        boot = open(os.path.join(KERNEL_ROOT, "raw_boot.bin"), "rb").read()
...
def build(verify_files=True):
    ...
    definitions = {
        ...
        "kernel_raw.elf": ("ELF32 raw-loader kernel", "tools/images/mkdisk.sh", "PARENT_DERIVED_PAYLOAD",
                           ["raw-bios"]),
        "BOOTX64.EFI": ("PE32+ native UEFI application", "buildefi.sh", "PARENT_BYTE_EXACT",
                        ["native-uefi64"]),
        "zlOS.iso": ("hybrid ISO with GRUB", "tools/images/mkiso.sh", "QEMU_DIRECT_ARTIFACT",
                     ["grub-bios32", "grub-uefi32"]),
        "zlOS64.iso": ("hybrid GRUB ISO with 64-bit multiboot payload", "tools/images/mkiso64.sh",
                       "QEMU_DIRECT_ARTIFACT", ["grub-bios64", "grub-uefi64"]),
        "zlOS.img": ("raw BIOS disk image", "tools/images/mkdisk.sh", "QEMU_DIRECT_ARTIFACT", ["raw-bios"]),
        "zlOS-usb.img": ("GPT/FAT UEFI USB disk image", "tools/images/mkusb.sh", "QEMU_DIRECT_ARTIFACT",
                         ["native-uefi64"]),
        "boot-media-ids.json": ("deterministic disk/FAT identity metadata", "tools/generators/gen-boot-media-ids.py",
                                "METADATA_ONLY", []),
    }
    artifacts = {}
    ...
    value = {
        ...
        "result": "PASS",
        "generator": {
            "path": "kernel/tools/generators/gen-artifact-registry.py",
            "sha256": sha256(os.path.abspath(__file__)),
        },
        "format_verifier": {
            "path": "kernel/tools/checks/check-elf-permissions.py",
        ...
        "application_evidence": {
            "path": "kernel/metadata/app-evidence.json", "sha256": sha256(APP_EVIDENCE),
