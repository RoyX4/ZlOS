...
#
#   src/kernel.zl -> ../compile -> out.c -> gcc -m32 -ffreestanding
#             +  boot/raw_entry.S (our entry, no multiboot)
#             +  the same console/font/runtime objects
#             -> boot/link-raw.ld (linked at 0x10000) -> objcopy -> kernel_raw.bin
#   boot/raw_boot.asm -> nasm -> raw_boot.bin (512-byte boot sector)
...
# Freestanding interpreter: language frontend/runtime sources + ksetjmp.
# shellcheck disable=SC2086
...
gcc -m32 -c boot/raw_entry.S -o _rawentry.o

# shellcheck disable=SC2086
ld -m elf_i386 -T boot/link-raw.ld -o kernel_raw.elf \
   _rawentry.o _gen.o _rt.o _gdt.o _smptr.o $OBJS
objcopy -O binary kernel_raw.elf kernel_raw.bin

nasm -f bin boot/raw_boot.asm -o raw_boot.bin
...
CHUNKS=$(grep -oP 'CHUNKS\s+equ\s+\K[0-9]+' boot/raw_boot.asm)
LIMIT=$((CHUNKS * 64 * 512))
