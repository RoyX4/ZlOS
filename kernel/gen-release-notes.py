...
ROOT = HERE.parent
JSON_OUTPUT = HERE / "release-notes.json"
MARKDOWN_OUTPUT = HERE / "docs" / "UNRELEASED-CHANGELOG.md"
INPUTS = (
    "build-identity.json",
...
def current_changes(decisions: dict) -> list[dict]:
    ...
    return [{
        ...
        "evidence": {
            ...
            "sha256": sha256(HERE / "decision-ledger.json"),
            "record_id": row["id"],
        },
...
def migrations() -> list[dict]:
    return [
        ...
        },
        {
            ...
            "verification": "kernel/hosttest/fbbench",
...
def known_issues(docs: dict[str, dict]) -> list[dict]:
    ...
    tests = docs["hosttest/test-run-receipt.json"]
    ...
    return [
        ...
        },
        {
            ...
            "source": "kernel/hosttest/test-run-receipt.json",
        },
        {
