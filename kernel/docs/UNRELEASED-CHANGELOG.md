...
Build identity: `7a01ae9c753071500184baf9a0b7c75f9fd161aeb165611f3c85e7e0cf582b6d`.
...
| DEC-0020 | USERS_AND_DEVELOPERS | yes | Recognize the physical I2C-HID transport, report decoder and compositor route while keeping touchpad feel gestures open. | The physical transport/decoder receipt does not promote tap, scroll, gesture feel or current-build hardware proof. |
...
- Verify: `kernel/verify-sources.sh`
...
- Verify: `kernel/tests/host/fbbench`
...
|---|---|---|---|
| ISSUE-001 | RELEASE_BLOCKER | Public redistribution authority is absent. | public_release_blocked=true, inputs_without_established_grant=146 |
...
- **REC-001 interrupted-sources-transaction (available):** detect with `kernel/verify-sources.sh --recover-only`; recover with `kernel/verify-sources.sh --recover-only`. The command must report a clean transaction before any build.
