...
> **Historical audit.** Recovered from the uncommitted
> `codex/full-restructure` worktree on 2026-08-25. Its measurements belong to
> commit `85fcdf5` plus that worktree's dirty restructure, not to current HEAD.
> Some paths were updated by the restructure author; the commands and line
> numbers are preserved as historical evidence and may not run against current
> HEAD. Every implementation claim requires current revalidation before use.
...
```
kernel/src/kernel.zl:2047     if cmd == 80 {              # the 'P' shell command
kernel/src/kernel.zl:2063         fb = panel_up()
freestanding/runtime_kernel.c:1976   "panel_up"  -> intel_bringup_panel()
kernel/src/drivers/display/intel.c:4372       u32 intel_bringup_panel(void)
kernel/src/drivers/display/intel.c:4417           intel_link_train_arm(1)
kernel/src/drivers/display/intel.c:4418           intel_modeset_run(0)
kernel/src/drivers/display/intel.c:4419           intel_link_train_arm(0)
kernel/src/kernel.zl:2083     while pk == 0 { pk = key_get()  idle() }
kernel/src/kernel.zl:2084     panel_console()  -> intel_panel_takeover()  (intel.c:4470)
```
...
grep -n 'panel_up\|panel_console' freestanding/runtime_kernel.c kernel/src/kernel.zl
```
...
```
$ python3 kernel/tools/checks/check-zlcalls.py
