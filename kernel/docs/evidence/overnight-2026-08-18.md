...
None of the three existing gates ever executed `kernel/boot/efi.c`:
...
**New: `kernel/tools/checks/verify-efi.sh`** boots `zlOS-usb.img` — our own `BOOTX64.EFI`,
...
`kernel/HANDOFF.md`, `kernel/src/drivers/display/intel.c`, `kernel/tests/host/` — the other session
