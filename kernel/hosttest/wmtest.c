...
int main(void)
{
     ...
     * no app draw routine. */
    {
        ...
        /* A terminal keystroke changes only its prompt row. The retained
         * client must update that strip in place; invalidating the full
         * 596x318 client was the measured physical typing-latency owner. */
        wm_client(under, &ux, &uy, &uw, &uh);
        unsigned int old_colour = APP_COLOUR(1);
        unsigned int new_colour = old_colour ^ 0x00000055u;
        draw_variant[1] = 0x00000055u;
        draw_calls[1] = 0;
        wm_invalidate_client_rect(under, 12, uh - 24, 96, 16);
        frame();
        ok("partial client invalidation redraws the app once",
           draw_calls[1] == 1);
        ok("partial client invalidation updates only the requested strip",
           fb_get_px(ux + 20, uy + uh - 20) == new_colour &&
           fb_get_px(ux + 20, uy + 20) == old_colour);
        wm_move(under, 180, 160);
        frame();
        wm_client(under, &ux, &uy, &uw, &uh);
        ok("the partially updated retained surface survives a move",
           fb_get_px(ux + 20, uy + uh - 20) == new_colour &&
           fb_get_px(ux + 20, uy + 20) == old_colour);
        draw_variant[1] = 0;
