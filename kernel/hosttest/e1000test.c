...
static int clear_reset = 1, complete_tx = 1;
...
int pci_vendor(int i) { return i == 0 ? 0x8086 : 0; }
int pci_device(int i) { return i == 0 ? 0x0d4f : 0; }
int pci_class(int i) { return i == 0 ? 2 : 0; }
int pci_subclass(int i) { return i == 0 ? 0 : -1; }
unsigned pci_bar(int i, int which) { return (i == 0 && which == 0) ? (unsigned)(uintptr_t)regs : 0; }
...
int main(void)
{
    ...
    prime();
    CHECK(e1000_find() >= 0, "target I219 not selected");
    CHECK(e1000_device() == 0x0d4f, "device %04x", e1000_device());
