...
DEFAULT_RECEIPT = os.path.join(HERE, "oracle", "out", "app-lifecycle-qemu.json")
...
def command_output(argv):
    return subprocess.check_output(argv, cwd=os.path.dirname(HERE), text=True).strip()
...
def run(receipt_path, no_build, timeout):
    ...
    source_paths = [
        os.path.join(HERE, "wm.c"),
        os.path.join(HERE, "kernel.zl"),
        os.path.join(HERE, "apps_registry.zl"),
        MANIFEST,
        BUILD_IDENTITY,
        os.path.join(HERE, "build_identity_embed.zl"),
    ...
    receipt = {
        ...
        "source_files_sha256": {os.path.relpath(path, HERE): sha256(path) for path in source_paths},
