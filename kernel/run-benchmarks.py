...
OUTPUT = HERE / "docs/receipts/benchmark-host-2026-08-23.json"
BENCHMARKS = (("wmbench", HERE / "hosttest/wmbench"), ("fbbench", HERE / "hosttest/fbbench"))
...
def execute() -> dict:
    ...
    for name, path in BENCHMARKS:
        ...
        executables.append({
            "name": name,
            "path": "kernel/hosttest/" + path.name,
            "sha256": sha256(path),
            "source_path": "kernel/hosttest/" + source.name,
    ...
    return {
        ...
        "runner": {"path": "kernel/run-benchmarks.py", "sha256": sha256(Path(__file__).resolve())},
    }
...
def validate_legacy_v1(value: dict) -> None:
    ...
    if value.get("build_identity") != json.loads((HERE / "build-identity.json").read_text())["identity_sha256"]:
    ...
    for executable in value.get("executables", []):
        ...
        executable_path = HERE / Path(executable["path"]).relative_to("kernel")
        source_path = HERE / Path(executable["source_path"]).relative_to("kernel")
...
def validate(value: dict) -> None:
    ...
    current_build_identity = json.loads((HERE / "build-identity.json").read_text())["identity_sha256"]
