# kernel/oracle — the fidelity oracle, zlOS half
...
`kernel/refrender/` (a different agent) renders the mockup to PNG. This
directory photographs zlOS and compares the two, per region.

```
kernel/refrender/out/reference-1280x800.png     <- the target
kernel/oracle/out/zlos-desktop.png              <- what we ship
kernel/oracle/regions.json                      <- 62 named boxes
kernel/oracle/diff-regions.py                   <- four numbers per box
```
...
nowhere else. The real desktop and its twenty-odd apps are in `kernel/kernel.zl`.
...
hash is `1302282882…` and it is byte-identical to `kernel/refrender/ds.html`,
