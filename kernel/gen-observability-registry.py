...
OUTPUT = HERE / "observability-registry.json"
BOOT_RECEIPTS = (
...
def build() -> dict:
    ...
    event_schema_path = HERE / "event-schema.json"
    ...
    host_receipt_path = HERE / "hosttest/test-run-receipt.json"
    ...
    idt_source = (HERE / "idt.c").read_text()
    crash_source = (HERE / "crash.c").read_text()
    ...
    value = {
        ...
        "host_test_receipt": {"path": "kernel/hosttest/test-run-receipt.json", "sha256": sha256(host_receipt_path)},
        "qemu_crash_receipt": {"path": "kernel/" + CRASH_RECEIPT,
                               "sha256": sha256(crash_path)},
        "event_schema": {"path": "kernel/event-schema.json",
                         "sha256": sha256(event_schema_path),
                         "result": event_schema["result"],
                         "target_emitters": 0},
        "source_identities": {path: sha256(HERE / path) for path in
                              ("idt.c", "crash.c", "crash.h", "heap.c",
                               "console.c", "support.c", "term.c", "verify-crash.py",
                               "hosttest/trace_event.c", "hosttest/trace_event.h",
                               "gen-event-schema.py",
