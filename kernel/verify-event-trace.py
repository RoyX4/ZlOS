...
RECEIPT = HERE / "docs/receipts/event-trace-host-2026-08-24.json"
SOURCES = ("hosttest/trace_event.h", "hosttest/trace_event.c",
           "hosttest/eventtracetest.c")
EXPECTED_CHECKS = 37
...
def run(argv: list[str], cwd: Path = HERE) -> subprocess.CompletedProcess[str]:
...
def validate(value: dict) -> None:
    ...
    expected = {name: sha256(HERE / name) for name in SOURCES}
    ...
    with tempfile.TemporaryDirectory(prefix="zlos-event-trace-") as temporary:
        ...
        value = {
            ...
            "build_identity": json.loads(
                (HERE / "build-identity.json").read_text())["identity_sha256"],
            "source_identities": {name: sha256(HERE / name) for name in SOURCES},
