...
OUTPUT = HERE / "accessibility-registry.json"
...
def build() -> dict:
    ...
    visual = json.loads((HERE / "visual-registry.json").read_text())
    rows = []
    ...
    value = {
        ...
        "source_identities": {
            path: sha256(HERE / path)
            for path in ("input.c", "ui.c", "wm.c", "hosttest/inputtest_hid.c", "hosttest/uitest.c", "hosttest/wmtest.c", "hosttest/toasttest.c")
        },
        "visual_registry": {
            "path": "kernel/visual-registry.json",
            "sha256": sha256(HERE / "visual-registry.json"),
