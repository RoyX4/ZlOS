...
ROOT = HERE.parent
OUTPUT = HERE / "build-graph.json"
IDENTITY = HERE / "build-identity.json"
ARTIFACTS = HERE / "artifact-registry.json"
TOOLCHAIN = HERE / "toolchain-manifest.json"
...
def build() -> dict:
    ...
    sources_list = [
        line.strip()
        for line in (HERE / "SOURCES").read_text().splitlines()
    ...
    node("meta:build-identity", "generated-metadata", path="kernel/build-identity.json")
    node("meta:app-manifest", "generated-metadata", path="kernel/app-manifest.json")
    ...
    for path in zl_sources:
        ...
        for module in IMPORT_RE.findall(text):
            imported = f"kernel/{module}.zl"
            if imported in source_hashes:
    ...
    lanes = (
        {
            ...
            "special": (
                ...
                ("kernel/gdt.c", "_gdt.o"),
                ("kernel/smp_trampoline.S", "_smptr.o"),
                ("kernel/boot.S", "_boot.o"),
            ),
        ...
        },
        {
            ...
            "special": (
                ...
                ("kernel/gdt64.c", "_gdt64.o"),
                ("kernel/smp_trampoline64.S", "_smptr64.o"),
                ("kernel/boot64.S", "_boot64.o"),
            ),
            "linker_script": "kernel/link64.ld",
        },
        {
            "id": "native-uefi64",
            ...
            "special": (
                ("kernel/efi.c", "_efi_efi.o"),
                ("freestanding/runtime_kernel.c", "_efi_runtime_kernel.o"),
                ("kernel/gdt64.c", "_efi_gdt64.o"),
                ("kernel/smp_trampoline64.S", "_efi_smptr.o"),
            ),
            "linker_script": None,
        },
        {
            "id": "raw-bios-loader32",
            "script": "kernel/mkdisk.sh",
            "artifact": "kernel_raw.elf",
            "special": (
                ("freestanding/runtime_kernel.c", "_rt.o"),
                ("kernel/gdt.c", "_gdt.o"),
                ("kernel/smp_trampoline.S", "_smptr.o"),
                ("kernel/raw_entry.S", "_rawentry.o"),
            ),
            "linker_script": "kernel/link-raw.ld",
        },
    )
    ...
    if "source:kernel/raw_boot.asm" in nodes:
        edge("source:kernel/raw_boot.asm", "artifact:zlOS.img", "boot-sector-input")
    ...
    if "source:kernel/gen-app-manifest.py" in nodes:
        edge("source:kernel/gen-app-manifest.py", "meta:app-manifest", "generates")
    if "source:kernel/app-manifest.json" in nodes:
        edge("source:kernel/app-manifest.json", "meta:app-manifest", "manifest-payload")
    if "source:kernel/gen-build-identity.py" in nodes:
        edge("source:kernel/gen-build-identity.py", "meta:build-identity", "generates")
    ...
    value = {
        ...
        "generator": {"path": "kernel/gen-build-graph.py", "sha256": sha256(Path(__file__).resolve())},
        "evidence_ceiling": (
