/* roots.c - the trust store.
 *
 * THREE ROOTS, NOT THE HUNDRED A BROWSER SHIPS. Mozilla's store is ~150
 * certificates and about 250 KB; this kernel's whole image is 1.6 MB. These
 * two are ISRG Root X1 and X2 - Let's Encrypt - which is what issues for
 * Wikipedia and for most of the plain web that a document browser would visit.
 *
 * THE CONSEQUENCE IS A REFUSAL, NOT A WARNING. A site chained to any other CA
 * fails to load and says why. That is the honest behaviour for a two-entry
 * store: silently trusting an unknown root would make the whole of x509.c
 * decorative, and a "continue anyway" button is how users are trained to click
 * through the one warning that mattered.
 *
 * Adding a root is adding its DER here. Do not add one without knowing what it
 * is - every certificate in this array can authenticate every site on the web.
 */
/* CN=ISRG Root X1,O=Internet Security Research Group,C=US */
static const unsigned char ROOT_X1[1391] = {
...
static const unsigned char ROOT_X2[543] = {
...
static const unsigned char ROOT_GTS[1371] = {
...
/* Parsed once, on first use. A parse failure leaves that slot out of the store
 * rather than aborting - a corrupt root should cost its own site, not all of
 * them. */
static struct x509_cert store[3];
static int nstore = -1;
...
const struct x509_cert *zl_roots(int *n)
{
    if (nstore < 0) {
        nstore = 0;
        if (x509_parse(ROOT_X1, (int)sizeof ROOT_X1, &store[nstore])) nstore++;
        if (x509_parse(ROOT_X2, (int)sizeof ROOT_X2, &store[nstore])) nstore++;
        if (x509_parse(ROOT_GTS, (int)sizeof ROOT_GTS, &store[nstore])) nstore++;
    }
    *n = nstore;
    return store;
}
