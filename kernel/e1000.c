/* e1000.c - bounded Intel wired Ethernet for QEMU 82540EM and ThinkPad I219.
 *
 ...
 * format and registers below are shared by 8254x and e1000e/I219 parts.  Every
 * wait is bounded, all DMA is explicit, and failure leaves the link unavailable
 * instead of making networking a boot dependency. */
...
{
    pci_scan(); dev_idx = -1;
    /* Exact target first; a dock or emulated adapter may also be present. */
    for (int pass = 0; pass < 2; pass++)
        for (int i = 0; i < pci_count(); i++) {
            int id = pci_device(i);
            if (pci_vendor(i) != 0x8086 || pci_class(i) != 0x02 ||
                pci_subclass(i) != 0x00 || !supported(id)) continue;
            if (pass == 0 && id != 0x0d4f) continue;
            dev_idx = i; dev_id = id;
