...
OUTPUT = os.path.join(HERE, "app-manifest.json")
...
def imported_app_paths(kernel_path, kernel):
    ...
    for module in modules:
        if not module.startswith("apps_"):
            continue
        path = os.path.join(os.path.dirname(kernel_path), module + ".zl")
        if not os.path.isfile(path):
            fail(f"kernel.zl imports missing app module {module}.zl")
        imported.append(path)
    discovered = set(glob.glob(os.path.join(os.path.dirname(kernel_path), "apps_*.zl")))
...
def build_manifest():
    ...
    registry_path = os.path.join(HERE, "apps_registry.zl")
