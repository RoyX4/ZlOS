<!-- Managed by tools/directory-docs.py. Edit only the LOCAL block. -->
<!-- BEGIN GENERATED: directory-docs -->
# kernel/tests/host

## Purpose

Host-compiled harnesses for kernel components and hardware-facing logic.

## Classification

- **Class:** `tests`
- **State:** active verification support
- **Evidence boundary:** Tests describe exercised behavior; they do not prove unexecuted routes or physical hardware.

## Start here

- Read the [project status](../../../docs/PROJECT-STATUS.md) for current truth.
- Read the local [status](STATUS.md), [tasks](TODO.md), [rules](RULES.md),
  [values](VALUES.md), and [operating procedure](SOP.md).
- Editing agents must also read [AGENTS.md](AGENTS.md).

## Child directories

- None.

## Direct tracked contents

- `apps53.py`
- `arenatest.c`
- `blocktest.c`
- `boot_handover_test.c`
- `boot_state_test.c`
- `browsershot.c`
- `browsertest.c`
- `build.sh`
- `chain_der.h`
- `crashtest.c`
- `cryptotest.c`
- `cssstub.c`
- `csstest.c`
- `dhcptest.c`
- `dnstest.c`
- `dpll_test.c`
- `e1000test.c`
- `easetest.c`
- `ecdsatest.c`
- `efi_kernel_witness_test.py`
- `efi_runtime_diag_test.py`
- `efi_stage0_test.py`
- `eventtracetest.c`
- `exectest.c`
- `fbbench.c`
- `fbtext.c`
- `forged.h`
- `fstest.c`
- `fuzz.c`
- `games12_rules.zl`
- `games4_rules.zl`
- `gpu-dev.sh`
- `gpu-ring-run.sh`
- `gpu_aperture.c`
- `gpu_blt.c`
- `gpu_fillrate.c`
- `gpu_planes.c`
- `gpu_ring.c`
- `gputest.c`
- `heaptest.c`
- `hostmachine.c`
- `hoststubs.c`
- `htmltest.c`
- `httptest.c`
- `i2c_touchtest.c`
- `i2ctest.c`
- `inputtest.c`
- `inputtest_feel.c`
- `inputtest_hid.c`
- `intel_probe.c`
- `iwlwifitest.c`
- `jmptest.c`
- `jstest.c`
- `killtest.sh`
- `libctest.c`
- `memmap-guard-test.sh`
- `memtypetest.c`
- `modeset-run.sh`
- `modeset_test.c`
- `msctest.c`
- `nettest.c`
- `pagingtest.c`
- `palette.c`
- `parsestat.c`
- `pngtest.c`
- `retest.c`
- `rtctest.c`
- `run-all.sh`
- `runtest.c`
- `settingstest.c`
- `systest.c`
- `tartest.c`
- `tcptest.c`
- `termwrap.c`
- `test-inventory.json`
- `test-policy.json`
- `test-run-receipt.json`
- `tlscryptotest.c`
- `tlstest.c`
- `toasttest.c`
- `trace_event.c`
- `trace_event.h`
- `tritest.c`
- `uitest.c`
- `walltest.c`
- `wmbench.c`
- `wmshot.c`
- `wmtest.c`
- `wmtest_feel.c`
- `x509test.c`
- `xhcitest.c`
- `zladdrs.sh`
- `zllog_e2e_test.py`
- `zllogtest.c`

This inventory is local orientation, not a completion claim. See the [code map](../../../docs/CODE-MAP.md)
for repository-wide ownership.
<!-- END GENERATED: directory-docs -->

<!-- BEGIN LOCAL: directory-docs -->
<!-- Add verified directory-specific notes here. -->
<!-- END LOCAL: directory-docs -->
