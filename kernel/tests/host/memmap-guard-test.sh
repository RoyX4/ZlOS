...
#
#   cd kernel/tests/host && ./memmap-guard-test.sh
#
...
OWNERS="src/graphics/framebuffer/fb.c
src/core/sched.c
src/drivers/input/i2c_hid.c
src/drivers/storage/nvme.c
src/drivers/input/xhci.c
src/drivers/display/virtio_gpu.c
src/core/arena.c
src/drivers/network/virtio_net.c
src/web/browser.c
src/drivers/display/intel.c"

pass=0; fail=0
work=$(mktemp -d) || exit 1
trap 'rm -rf "$work"' EXIT INT TERM

# A private source-tree copy whose memmap.h can be rewritten safely.
mkroom() {
    room="$work/$1"; rm -rf "$room"; mkdir -p "$room"
    cp -R "$KDIR/src" "$room/src"
}

# compile_all <room> -> prints diagnostics, returns 0 if every owner compiled
compile_all() {
    ( cd "$1" && INCLUDES=$(find src -type d -printf ' -I%p' | sort)
      for c in $OWNERS; do
          gcc $CFLAGS $INCLUDES -fsyntax-only "$c" 2>&1 || echo "FAILED:$c"
      done )
}
...
expect_break() {
    mkroom broken
    sed -i "$2" "$work/broken/src/arch/x86/memmap.h"
...
expect_break "the real bug: HID_BUF back at 0x0C900000, inside the blur arena" \
    ...
    "i2c_hid: buffers escape their region"
...
# The current ceiling is 1 GiB. Shrink the promise below the fixed 241..255 MiB
# virtio-gpu framebuffer and require the device owner's guard to name it. The
# former mutation moved HI_VGPU from 240 to 249 MiB; after HI_TOP became 1 GiB
# that no longer crossed anything and this canary silently stopped canarying.
expect_break "guest RAM promise shrunk below virtio-gpu's framebuffer" \
    's/^#define HI_TOP    0x40000000UL/#define HI_TOP    0x0FE00000UL/' \
...
same() {
    ...
    out=$( cd "$work/same" && INCLUDES=$(find src -type d -printf ' -I%p' | sort)
           gcc $CFLAGS $INCLUDES -fsyntax-only "$file" 2>&1 )
...
same src/core/sched.c \
    "STACK_BASE == 0x0B000000u" "COUNTER_BASE == 0x0B0F0000u"

same src/drivers/storage/nvme.c \
...
same src/drivers/display/virtio_gpu.c \
...
same src/graphics/framebuffer/fb.c \
...
same src/drivers/network/virtio_net.c \
