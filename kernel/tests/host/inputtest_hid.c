/* inputtest_hid.c - the KEYBOARD half of input.c, against fake hardware.
 *
 * This arrived as claude/ecstatic-lewin-f617bb's rewrite of inputtest.c. It was
 * not merged into that file and it is not a duplicate of it: the two test
 * disjoint halves of the same source. inputtest.c covers the pointer and the
 * serial source - EV_MOUSE coalescing, button masks, "a parked pointer is
 * silent", serial bytes arriving as characters with no key event beside them.
 * This one covers the keyboard - PS/2 and USB arrows, shift/ctrl/caps
 * composition, key-held and repeat.
 *
 * Merging them would have meant reconciling two different stub layers (70
 * lines against 255) for no coverage gain, and taking either alone silently
 * dropped about twenty assertions. Two binaries, both built, nothing lost.
 */
...
/* input.c on main also drives the pointer and the serial source; this
 * harness only exercises the keyboard, so these are inert. A pointer that is
 * never ready keeps pump_mouse() on the PS/2 path, and a PS/2 mouse parked at
