...
int main(void)
{
    ...
    ok("mapping three pages at 1 MiB succeeds", intel_ggtt_map_range(256u, 0x10000000u, 3) == 1);
    ...
    ok("a second range while one is held is refused", intel_ggtt_map_range(600u, 0x20000000u, 1) == 0);
    ...
    ok("a range past the table is refused", intel_ggtt_map_range(entries - 2u, 0x10000000u, 3) == 0);
    ok("...holding nothing", intel_ggtt_saved_pages() == 0);
    ok("a range longer than the save area is refused", intel_ggtt_map_range(0u, 0x10000000u, 4097) == 0);
    ok("...holding nothing", intel_ggtt_saved_pages() == 0);
    ...
    ok("mapping again after restore succeeds", intel_ggtt_map_range(256u, 0x10000000u, 2) == 1);
