...
KERNEL = os.path.dirname(TESTS_DIR)
PROBE_DIR = os.path.join(KERNEL, "tools", "probes")
...
def catalog_apps():
    ...
    """
    src = open(os.path.join(KERNEL, "apps", "apps_registry.zl")).read()
    ...
    for path in sorted(_glob.glob(os.path.join(KERNEL, "apps", "apps_*.zl"))):
...
def variant_source(want_w, want_h, dest):
    ...
    """
    src = open(os.path.join(KERNEL, "src", "kernel.zl")).read()
