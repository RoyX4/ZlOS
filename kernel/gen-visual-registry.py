...
OUTPUT = HERE / "visual-registry.json"
PATTERNS = (
    "virtio-gpu-proof.png",
    "shots/*.png",
...
def classification(relative: str) -> tuple[str, str]:
    if relative.startswith("hosttest/"):
...
def discover() -> list[Path]:
    ...
    return sorted(set(paths), key=lambda path: path.relative_to(HERE).as_posix())
...
def validate(value: dict) -> None:
    ...
    expected_paths = [path.relative_to(HERE).as_posix() for path in discover()]
