/* font8x16.c - an 8x16 bitmap font, generated from
 * /usr/share/consolefonts/alt-8x16.psf.gz by mkfont.py.
 *
 * UEFI gives you a linear framebuffer and no text mode, so glyphs have
 * to be drawn pixel by pixel. One byte per row, high bit leftmost.
 * Only printable ASCII 32..126 is kept - a kernel that cannot print
 * anything is a worse problem than one that cannot print box drawing. */
