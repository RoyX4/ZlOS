...
ROOT = HERE.parent
OUTPUT = HERE / "toolchain-manifest.json"
IDENTITY = HERE / "build-identity.json"
DEPENDENCIES = HERE / "dependency-lock.json"
...
def run(argv: list[str], cwd: Path = HERE) -> str:
    try:
...
def dependency_paths(tool: str, flags: list[str], source: str) -> list[Path]:
    ...
    result = []
    for value in values:
        path = (HERE / value).resolve() if not Path(value).is_absolute() else Path(value).resolve()
        try:
...
def build() -> dict:
    ...
    scripts = {
        "bios32": HERE / "build.sh",
        "bios64": HERE / "build64.sh",
        "uefi64": HERE / "buildefi.sh",
        "raw32": HERE / "mkdisk.sh",
    }
    required_script_shape(scripts["bios32"], ("-mgeneral-regs-only", "ld -m elf_i386 -T link.ld"))
    required_script_shape(scripts["bios64"], ("-mgeneral-regs-only", "ld -m elf_x86_64 -T link64.ld"))
    required_script_shape(scripts["uefi64"], ("-mgeneral-regs-only", "/Brepro /lldignoreenv", "-subsystem:efi_application"))
    required_script_shape(scripts["raw32"], ("-mgeneral-regs-only", "ld -m elf_i386 -T link-raw.ld", "objcopy -O binary"))
    ...
    lanes = [
        {
            ...
            "linker": "ld",
            "linker_flags": ["-m", "elf_i386", "-T", "link.ld"],
            "format": "ELF32-i386",
        ...
        },
        {
            ...
            "linker": "ld",
            "linker_flags": ["-m", "elf_x86_64", "-T", "link64.ld"],
            "format": "ELF64-x86-64",
        ...
        },
        {
            ...
            "linker_flags": ["-m", "elf_i386", "-T", "link-raw.ld", "-O", "binary"],
    ...
    probes = (
        ("bios32-fb", "gcc", gcc32, "fb.c"),
        ("bios64-fb", "gcc", gcc64, "fb.c"),
        ("uefi64-fb", "clang", clang_uefi, "fb.c"),
        (
