        ...
        if (mphase == 0 && !(b & 0x08)) {
            ...
            if (mphase == mouse_pktlen) {
                ...
                if (!(flags & 0xC0)) {         /* ignore overflowed packets */
                    ...
                    /* THE WHEEL FIELD IS 4-BIT SIGNED, not a byte. Bits 3:0
                     * are the notch count in two's complement; 4..7 are
                     * buttons 4 and 5 on the mice that have them. Reading the
                     * whole byte makes one notch backwards (0x0F) look like
                     * fifteen notches forwards - which is the entire bug this
                     * field invites, and it only shows when you scroll UP. */
                    if (mouse_pktlen == 4) {
                        int wz = mpkt[3] & 0x0F;
