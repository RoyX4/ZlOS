...
OUTPUT = os.path.join(HERE, "build-identity.json")
EMBED = os.path.join(HERE, "build_identity_embed.zl")
IMPORT_RE = re.compile(r"^\s*import\s+([A-Za-z_]\w*)\s*(?:#.*)?$", re.M)
ROUTE_INPUTS = (
    "SOURCES", "build.sh", "build64.sh", "buildefi.sh", "mkdisk.sh",
    "mkiso.sh", "mkiso64.sh", "mkusb.sh", "link.ld", "link64.ld", "link-raw.ld",
    "boot.S", "boot64.S", "raw_entry.S", "raw_boot.asm", "efi.c",
    "gdt.c", "gdt64.c", "smp_trampoline.S", "smp_trampoline64.S",
    "app-manifest.json", "app_manifest_embed.zl",
    "gen-app-manifest.py", "gen-build-identity.py",
    "gen-boot-media-ids.py",
...
def zl_imports():
    pending = [os.path.join(HERE, "kernel.zl")]
    result = set()
    while pending:
        ...
        for module in IMPORT_RE.findall(text):
            ...
            if module == "build_identity_embed":
                continue
            child = os.path.join(HERE, module + ".zl")
...
def input_paths():
    ...
    for line in read_bytes(sources_path).decode().splitlines():
        ...
        if value and not value.startswith("#"):
            paths.add(os.path.join(HERE, value))
    for name in ROUTE_INPUTS:
        paths.add(os.path.join(HERE, name))
    for directory in (HERE, os.path.join(ROOT, "freestanding")):
...
def validate(value):
    ...
    required = {"kernel/kernel.zl", "kernel/SOURCES", "kernel/app-manifest.json",
                "kernel/app_manifest_embed.zl", "kernel/tools/reproducible_time.c",
                "kernel/gen-boot-media-ids.py",
...
def selftest(value):
    mutations = []
    for name, mutate in (
        ("missing-source", lambda x: x["source_files_sha256"].pop("kernel/kernel.zl")),
