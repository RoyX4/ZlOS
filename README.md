...
> **Complete research-to-implementation program:**
> [`docs/program/README.md`](docs/program/README.md). This is the exhaustive
> 906-feature, 609-target, 174-contract destination and dependency graph. The
...
- [`docs/REPOSITORY-STRUCTURE.md`](docs/REPOSITORY-STRUCTURE.md) — why the layout
  looks this way and where new work belongs.
- [`src/README.md`](src/README.md) — ownership inside the host language toolchain.
...
2. **Real `kill`/`start`/`rm`/`copy`/`move`/`run`** in
   `src/runtime/runtime.c` and `src/runtime/interp.c`.
...
is load-bearing: `src/frontend/lexer.c:272-273` discards comments and
`src/frontend/lexer.c:88` truncates
...
machine code with no interactive debugger in the loop.
`src/backends/native/nativegen.c` *is* fully
ported and tested.
