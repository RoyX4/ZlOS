...
- **`switch` is not a reserved word.** It is recognised positionally, the same
  way `do` and `loop` already are (`parser.c:905-908`).
...
- **Implementation is a desugaring in `parser.c`** into the `assign` + `if` /
  `elif` tree that already exists — so `interp.c` and `compile.c` need **zero
  changes**, exactly as with `for i = a to b`, `do/while` and `loop`.
- Two of the four backends (`compilef.c`, `nativegen.c`) need a **three-line**
...
zl already has `if`/`elif`/`else` (`parser.c:574-601`), a ternary
(`parser.c:537-549`), and `in` / `not in` (`parser.c:457-487`). Any switch can
...
- **`compiler.zl:36-37` is a twelve-way `or` chain** on one line, testing one
...
- **A dispatch reads as a table, not as a decision tree.** `if tag == "if" ...
  elif tag == "while" ... elif tag == "for"` (`compiler.zl:550-565`) repeats the
...
`lexer.c:29-45` is unusually explicit about the cost of a keyword:

> *A keyword is a word the language steals from you. [...] Every word added here
> is a name your users lose forever.*

The list is fifteen words (`lexer.c:48-54`). `design_modules.md` §2 turned down
...
`parser.c:905-908` shows the existing technique:
...
rule at `parser.c:623-626` and it is the thing to preserve.
...
| Keeps `lexer.c` and `compiler.zl:36` keyword lists in sync | **no** (see §8) | yes | yes |
...
and because `T_NEWLINE` is a real token (`lexer.c:267-268`), so an expression
...
So the invariant at `parser.c:623-626` holds exactly, and `switch` stays a legal
...
`compiler.zl`'s hand-written lexer loop. It costs only `switch (a + b) {`, which
...
`else` is already a keyword (`lexer.c:49`) and already reads as "otherwise".
...
character (`lexer.c:224`), so `_` is an ordinary variable name and a `_` arm
...
when `values_equal(subject, arm_value)` (`interp.c:1126-1141`). That function
...
made — blocks do not create scopes today (`interp.c:1505-1507` runs an `N_BLOCK`
in the *same* `Env`; a new `Env` appears only per function call,
`interp.c:1279`). Designing switch around a feature that does not exist would
mean designing scope by accident. **Out of scope.**

**Ranges.** `1 to 5` is tempting because `to` already exists positionally in
for-headers (`parser.c:731-735`). But: it only works when the subject is
...
only on the for-range path (`parser.c:623-626`) — it would be the first place
...
1. **There is nothing to gain.** The point of constant labels is a jump table.
   zl numbers are C `double`s (`runtime.h`, `interp.c` `Value.num`) and zl case
...
`break` sets a flag (`interp.c:1468`) that the enclosing `while`/`for` consumes,
and `compile.c:380` emits a literal C `break;`. Under the proposed desugaring a
...
- **Fallthrough cannot be desugared.** An `if`-chain has no way to enter arm
  ...
  *k* — which is O(N²) tree growth and puts the same `Node` under two parents.
  `parser.c:940-947` records exactly why that is unacceptable: the old
  ...
  zl has no notion of and which `nativegen.c` and `compilel.c` would each have
  to grow labels for.
...
(`interp.c:1180`). An expression switch therefore needs either a new
...
into `__zl_lim0`/`__zl_st0` (`parser.c:772-776`). An expression appears in the
...
supports `?:` already handles (`compile.c:259-266`). It would be additive. It is
...
- **Compare numbers by value, not by text.** `1`, `1.0`, `01` and `1e0` are four
  ...
  `l.num == r.num` (`interp.c:1130-1131`), so the duplicate check must
...
- **Compare strings after escape processing.** The lexer resolves `\n`, `\t`,
  `\xNN` while producing the token (`lexer.c:180-215`), so `n->text` is already
...
`parser.c:614-627` states the technique and why it is used:
...
|---|---|---|
| the hidden name `__zl_sub0` | `hidden_ident()` (`parser.c:636-641`) | `__zl_lim0`, `__zl_st0`, `__zl_first0`, `__zl_again0` |
| `__zl_sub0 = SUBJ` | `assign_to()` (`parser.c:673-679`) | for-range, do-while |
| `__zl_sub0 == A` | `binary()` (`parser.c:664-671`) | for-range's bounds test |
| the arm as an `N_IF` | plain `new_node(N_IF)` | `parse_if` |
| chaining arms | `n->c` = an `N_BLOCK` holding the next `N_IF` | **`parse_if`'s own `elif` handling, `parser.c:581-589`** |
| wrapping assign + chain into one statement | `N_BLOCK` | for-range (`parser.c:772-778`), do-while (`parser.c:843-846`) |
...
(`parser.c:912-925`) next to the existing `starts_block_word` checks.

**Everything downstream is untouched.** `interp.c` walks `N_ASSIGN`/`N_IF`
(`interp.c:1461-1467`); `compile.c` emits them (`compile.c:343-349`);
`compile.c`'s `collect_vars` picks up `__zl_sub0` automatically because it is a
...
`parser.c:815-817` already says why, in the do-while comment:

> *(A plain `or` would not do - the C backend evaluates both of its operands.)*

`interp.c:1337-1339` short-circuits `and`/`or` in `eval`; `compile.c` routes
...
`compile.c:259-266` states the guarantee:
...
§7.7. Both `parse_for_range` (`parser.c:758-761`) and `parse_do_while`
(`parser.c:834-837`) already use a ternary for precisely this reason, so this is
...
(`interp.c:1211-1218`), which means (a) all the element expressions are
evaluated regardless — same divergence as `or`, and (b) an `N_LIST` literal
allocates a `Value` per element on every evaluation (`interp.c:1327-1334`), so a
...
Don't: `parser.c:940-947` documents the bug that came from putting one `Node` in
...
`compiler.zl:36-37` costs *today* as a hand-written `or` chain — so switch is
...
The one real scaling cost is **nesting in the emitted C**. `compile.c:347` emits
...
`compile.c` — when an `N_IF`'s else-block is a single `N_IF`, emit `else if (`
...
1. **The tree dump shows the rewrite, not the source.** `print_node`
   (`parser.c:1061` region, used by `parser.exe`) will print nested `N_IF`s and a
...
3. **The hidden variable is a real variable.** Blocks do not create scopes
   (`interp.c:1505-1507`), so `__zl_sub0` becomes an ordinary function-local or
   ...
   construct counter (`parser.c:632-641`) is the whole defence, and it is the
   same defence the loop forms already rely on.
4. **`g_hidden` must stay deterministic.** It is reset per `parse()` so the same
   source yields the same tree (`parser.c:630-632`) — which is what keeps the
...
`NodeType` in `parser.h`, an arm in `interp.c`, an arm in `compile.c`, an arm in
`compilel.c`, an arm in `nativegen.c`, an arm in `compilef.c`, a new tree tag in
`compiler.zl`'s parser **and** a new arm in `compiler.zl`'s codegen — eight
...
language (`interp.c`, `compile.c`). It is not true of the other three, and this
document should not pretend otherwise.

| Backend | Handles `N_BLOCK` as a statement? | Handles `N_TERNARY`? |
|---|---|---|
| `interp.c` | yes (`:1506`) | yes (`:1372`) |
| `compile.c` | yes (`:376`) | yes (`:259`) |
| `compilel.c` (LLVM IR) | yes (`:244`) | **no** — falls to the error exit at `:176` |
| `compilef.c` (unboxed C) | **no** | **no** — error exits at `:79` and `:120` |
| `nativegen.c` (x86-64 PE) | **no** | **no** — error exit at `:277` |
...
2. **Single-value-arm switches need only `N_BLOCK`.** The ternary appears only
   ...
   to `compilef.c`'s `emit_stmt` and the equivalent to `nativegen.c`'s
   `gen_stmt` — three lines each, and `nativegen.c:282-289` already has the
...
branch-and-phi in `compilel.c`, a conditional jump in `nativegen.c`) and should
...
## 8. What happens to `compiler.zl`

`compiler.zl` is zl's compiler written in zl, and it implements a **subset** of
the surface syntax. Concretely, today:

- its keyword list (`compiler.zl:36-37`) has **twelve** words against the C
  lexer's fifteen — no `elif`, no `break`, no `continue`;
- `parse_if` (`compiler.zl:309-319`) handles `else` but **not `elif`**;
- `parse_statement` (`compiler.zl:366-389`) handles only `if`, `while`, `for`,
...
`f(f(x)) == f(x)`: the interpreter runs `compiler.zl` to produce `gen1.c`, that
builds a compiler, and that compiler recompiles `compiler.zl` to `gen2.c`, which
must be byte-identical. A `parser.c` change cannot move that, because
`compiler.zl`'s *source* is unchanged and `compiler.zl`'s own parser is what
produces `gen2.c`. The fixpoint holds as long as **`compiler.zl` itself does not
...
`zlc.exe` built from `compiler.zl`. That is the existing two-tier situation, and
...
is the sharpest argument in §2.1 and it belongs here. `lexer.c:46-62` and
`compiler.zl:36-37` are two independent copies of the keyword list. Make
...
`verify.ps1` cannot catch (the fixpoint only checks `compiler.zl` against
itself). Recognising `switch` positionally means the keyword lists stay
identical and the question never arises.

**Teaching `compiler.zl` switch, when the time comes, is small** — and this is
...
`["block", ...]`, all of which `compiler.zl`'s codegen already handles
(`compiler.zl:550-565`), the work is **one `parse_switch` function of ~40 lines
in `compiler.zl`'s parser and zero lines in its codegen.** A real `N_SWITCH`
...
| File | Change |
|---|---|
| `lexer.c` | **none** — no new keyword, no new symbol |
| `parser.h` | **none** — no new `NodeType` |
| `parser.c` | `leads_switch()` forward-scan predicate (~12 lines) next to `starts_block_word` (`:905`); `parse_switch()` (~70 lines) next to `parse_do_while` (`:818`); one dispatch line in `parse_statement` (`:924`) |
| `interp.c` | **none** |
| `compile.c` | **none** (optional 4-line `else if` peephole, §7.4) |
| `compilef.c` | `case N_BLOCK:` in `emit_stmt` (3 lines) — also fixes for-range/do-while/loop there |
| `nativegen.c` | `case N_BLOCK: gen_block(n); break;` in `gen_stmt` (3 lines) — same |
| `compilel.c` | **none** (already has `N_BLOCK` at `:244`) |
| `compiler.zl` | **none now.** Must not use `switch` until it learns it (§8) |
| `docs/REFERENCE.md` | document the form; mark it C-toolchain-only |
| `tests/test_syntax.zl` | the programs in §10 |

Roughly 90 new lines of C, all in `parser.c`, plus six lines across two
backends.
...
8. **Nested switches** — asserts `g_hidden` numbering keeps `__zl_sub0` and
   `__zl_sub1` apart, the same property `parser.c:629-632` guarantees for nested
   for-ranges.
...
- **Forward scan or one-token guard (§2.1)?** Recommended: forward scan, because
  it preserves the `parser.c:623-626` invariant exactly. The guard is the
  fallback if the scan proves awkward to mirror in `compiler.zl` later.
...
- **The `else if` peephole in `compile.c` (§7.4)?** Not now. Do it when a
