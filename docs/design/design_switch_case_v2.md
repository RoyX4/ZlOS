...
Line citations were re-checked against the tree on 2026-08-02, but `parser.c`,
`lexer.c` and the numeric backends were being edited concurrently (floats), so
...
| 4 | Cases are values, compared with `==` | **Add a hazard: a map-valued arm will silently never match** | `values_equal_depth`'s `default: return 0` (`interp.c:1236-1256`) will swallow a new `V_MAP` |
| 5 | Teaching `compiler.zl` is "~40 lines in the parser and **zero** in codegen" | **Wrong. Its `emit_stmt` has no `"block"` tag** | Verified: `compiler.zl:536-574` |
| 6 | Ship two three-line backend additions with this change | **Still right, and the debt is now one release older** | The loops shipped without them; `compilef.c` and `nativegen.c` still reject `N_BLOCK` |
...
observation. `parser.c:614-627` is the standing statement of the technique:
...
`parse_*` function each and **no `NodeType`, no `interp.c` arm, no `compile.c`
...
| `interp.c` | yes | yes |
| `compile.c` | yes | yes |
| `compilel.c` | yes (`:282`) | **no** — `default:` error at `:214` |
| `compilef.c` | **no** — `emit_stmt` (`:86`) `default:` error at `:120` | **no** — expr `default:` at `:79` |
| `nativegen.c` | **no** — `gen_stmt` (`:203`) `default:` error at `:276-278` | **no** — same `default:` |

`nativegen.c:282-289` already has the helper the fix would call:
...
`values_equal_depth` compares lists structurally (`interp.c:1236-1256`):
...
`interp.c:1256`, mirrored at `runtime.c:213-233`. A new `V_MAP` that does not
...
- **`design_maps_v2.md` must state whether `==` on maps is defined**, and if it
  ...
  between `interp.c` and `runtime.c` is load-bearing (both were verified
...
**`interp.c` — never.** Each arm test is an `eval` of an `N_BINARY` reaching
`values_equal` (`interp.c:1330`). A table would need the subject hashed and an
...
**`compile.c` / `runtime.c` — never, and the chain is not the cost.** Every `==`
becomes a `zl_binop` call, and `zl_binop` dispatches on the operator **by
string** (`runtime.c:314-329`):
...
**`compilel.c` (LLVM) — the crossover is 4, and it is not ours.** LLVM's
...
is treated as fact: compile a 12-arm integer switch through `compilel.c`, dump
...
**`nativegen.c` — never, by policy.** `MASTER_PLAN.md` §10 says nativegen is
...
where that lands: `compiler.zl:36-37` is a twelve-way string dispatch,
...
twelve words against the C lexer's fifteen (`lexer.c:46-62`). So zl's single
...
`N_SWITCH` is a `NodeType` in `parser.h` plus arms in `interp.c`, `compile.c`,
`compilel.c`, `compilef.c`, `nativegen.c`, plus a tree tag *and* a codegen arm
in `compiler.zl` — eight places that must agree, against a three-engine gate.
...
## 6. `compiler.zl` — correcting v1 §8

v1 §8's conclusion was:

> the work is **one `parse_switch` function of ~40 lines in `compiler.zl`'s parser
> and zero lines in its codegen.**

**The "zero lines in its codegen" half is wrong.** `compiler.zl:536-574`,
...
exists (`compiler.zl:528`) but is only ever called from the `if`, `while` and
...
teaching `compiler.zl` switch needs **a `"block"` arm in `emit_stmt` as well** —
...
- `compiler.zl`'s assign arm is `"v_" + node[1] + " = " + emit_expr(node[2])`
...
as long as `compiler.zl` does not itself use `switch` (`compiler.zl:366-389`,
...
keyword lists from drifting (`lexer.c:46-62` has 15 words, `compiler.zl:36-37`
...
| `lexer.c` | none | — |
| `parser.h` | none | — |
| `parser.c` | `leads_switch()` forward scan (~12); `parse_switch()` (~75, up from 70: the identifier/hoist branch); duplicate detection with `strtod` (~30); one dispatch line at `parse_statement` (`:911-925`) | +15 |
| `interp.c` | none | — |
| `compile.c` | none | — |
| `compilef.c` | `case N_BLOCK:` in `emit_stmt` (`:86`) — **ship separately, before switch** | moved out |
| `nativegen.c` | `case N_BLOCK: gen_block(n); break;` in `gen_stmt` (`:203`) — same | moved out |
| `compilel.c` | none | — |
| `compiler.zl` | none now. Later: `parse_switch` (~45) **and a `"block"` arm in `emit_stmt` (~3)** | v1 said 0 in codegen |
...
**Size: ~120 lines of C, all in `parser.c`** (v1 said ~90; the extra is the
...
`compiler.zl` whenever the self-hosted compiler catches up.
...
12. **Identifier subject emits no hidden variable** — `switch x { 1 {} }`
    compiled by `compile.c`; assert `out.c` contains no `__zl_sub`. This is the
...
- **Floats.** Another workflow is adding floats to the fast backends. Numeric
  case arms are compared as C `double`s (`interp.c:1240-1241`), so `switch`
...
- **`switch` on the LHS of the forward scan in `compiler.zl`.** v1 §2.1 offered
  the one-token guard as a fallback if the forward scan is awkward to mirror in
  `compiler.zl`'s hand-written token loop. Having now read that loop
  (`compiler.zl:135-165` — `kind()`, `text()`, `advance()`, `is_sym()`), a
