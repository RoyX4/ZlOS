...
`docs/archive/superseded/INTEGRATION-PLAN.md`, which was written earlier against a base that turned out
to be the wrong one.
...
|---|---|---|
| `kernel/src/graphics/windowing/wm.c` overnight | +1149/**-0** | +354/**-22** |
| `kernel/src/graphics/windowing/wm.c` feel | +1091/**-0** | +289/**-15** |
| `kernel/src/graphics/windowing/term.c` overnight | +277/**-0** | +84/**-18** |
...
| D | dma-map (`kernel/src/arch/x86/memmap.h`) | HI_BACK 128 / HI_HID 184 / HI_BLUR 192 MiB |
...
`kernel/tools/checks/check-memmap.sh` as it stands **cannot** catch §2.1: it iterates a
