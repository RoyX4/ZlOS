...
The harness says so itself, and says it backwards. `kernel/tools/probes/exercise.py:308`:
...
`kernel/tests/host/xhcitest.c` compiles the shipping `xhci.c` and `input.c`
...
`kernel/src/arch/x86/idt.c:150` declares
...
`kernel/tests/host/xhcitest.c`, wired into `hosttest/build.sh` and therefore into
