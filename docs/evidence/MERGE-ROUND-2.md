...
[`MERGE-EVIDENCE.md`](MERGE-EVIDENCE.md). [`docs/archive/superseded/INTEGRATION-PLAN.md`](../archive/superseded/INTEGRATION-PLAN.md)
...
that got `docs/archive/superseded/INTEGRATION-PLAN.md` marked SUPERSEDED ("written against the wrong
base").
...
|---|---:|
| `kernel/src/kernel.zl` | **9** |
| `docs/JOURNAL.md` | 7 |
| `TODO.md` | 7 |
| `kernel/src/graphics/windowing/wm.c` | 6 |
| `kernel/src/graphics/framebuffer/fb.c` | 5 |
| `freestanding/runtime_kernel.c` | 5 |
| `kernel/src/graphics/windowing/term.c`, `raw_boot.asm`, `browser.c`, `mkdisk.sh`, `check-memmap.sh`, `probe-files.py`, `HANDOFF.md` | 4 |
...
1. `.gitignore` the noise: `.cache/clangd/`, `.cursor/`, and the four untracked
   `kernel/tests/host/` binaries (`fbtext`, `palette`, `termwrap`, `walltest` —
   build outputs; `kernel/tests/host/inputtest_feel` and `wmtest_feel` are already
...
```bash
kernel/tools/checks/check-memmap.sh     # address collisions
kernel/tools/checks/check-zl-calls.sh   # zl call sites with no builtin — zl has no linker
kernel/tools/checks/verify-iso.sh       # boots zlOS.iso, not kernel.elf
```
...
Same as round 1, and worse: **`kernel/src/kernel.zl`, now contested by nine
...
would delete `kernel/src/arch/x86/usermode.c` (420 lines), `verify_selfhost.sh`, and most of
...
- `kernel/tests/host/cryptotest` is still a tracked ELF with no build script.
...
| `kernel/tools/checks/check-memmap.sh` | exit 0 |
| `kernel/tools/checks/check-zl-calls.sh` | exit 0 |
| `kernel/tools/checks/verify-iso.sh` | **exit 0 — BIOS and UEFI, compositor 3 windows** |
| `kernel/tools/checks/verify-disk.sh` | **exit 0 — BOOTCOUNT 1 -> 2 -> 3, file survives by name** |
...
port every app onto the theme roles"). `kernel/src/arch/x86/usermode.c` is still 420 lines,
...
1. **The corrupt Zaccoding example did not lex.** 8 bytes beginning with U+FFFD and
