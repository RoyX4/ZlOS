...
- Intel: PTE save/restore in teardown (only relocation landed) - **closed 2026-09-08:** `intel_ggtt_map_range` saves every entry it is about to overwrite (one range at a time, refused rather than partially saved past 4096 pages or the table's end) and `intel_ggtt_restore` writes them back lo-cleared, hi, lo after the transcoder is off in the teardown and on the early-failure path where the firmware is still scanning; `ggttest` holds it against a 16 MiB fake BAR0 (21 assertions, byte-identical table after restore). PSR restore
  order in teardown (suspected) - **not changed, 2026-09-08, and here is why:** the teardown restores `EDP_PSR_CTL` to the saved value at plan section 3 step 4, before the plane and transcoder go off; the comment above `intel_modeset_teardown` records that the alternative ("disable it again") hung i915 for twenty minutes and cost a power cycle when the harness rebound the driver. The suspicion (re-enabling PSR on a transcoder about to be disabled) is plausible and untested; the only instrument that can settle it is the laptop's host loop (`modeset-run.sh`), not QEMU, so the measured order stays until then. `intel_dpll_program_*` and the AUX stack
  are still ungated - the CLAUDE.md table now says so.
- xHCI: no disconnect handling (modifiers stick on unplug) - **closed 2026-09-06:** `xhci_poll` now handles Port Status Change Events: a port whose CCS cleared detaches the keyboard (every held key released into the event queue, then `KEV_MOD(0)`, `kbd_mods` = 0, `kbd_ready` = 0) or the pointer (button released, `ptr_ready` = 0) and acknowledges the RW1C bits through `portsc_keep`; `xhcitest` "unplug releases held keys and modifiers" is red on the old driver (8 of 13 assertions) and green now. Re-plug is not automatic: `xhci_bringup` re-enumerates a not-ready device only when `usb_up()` runs again, and `kernel.zl` calls that at boot (`usb_boot`) and from the Terminal `usb` and `/` commands, never periodically - so a plugged-back keyboard needs the `usb` command typed (from the PS/2 or a second keyboard). **Closed 2026-09-08:** the unplug now gives the slot back (`xhci_forget_port`), a plug is remembered per port and enumerated from `xhci_poll` after a 120 ms debounce; the command and transfer waiters DEFER Port Status Change Events for the poll instead of discarding them (measured on the UEFI route: the plug event landed inside the unplug's own Disable Slot wait and was dropped as noise); and a 0.5 s PORTSC scan catches a plug the ring never announced. Witness: `verify-usb-replug.py` (QEMU `device_del`/`device_add`, no guest input between, read through the new read-only `usbstat` word) - PASS on native-uefi64 and bios32, receipts under `kernel/docs/receipts/usb-replug-*`. A same-day adversarial pass over this batch (a fresh reviewer, briefed to refute the commit messages) found six more, all reproduced and fixed before landing: the GGTT save hold refused the compositor's second mapping (`gputest` stubs the real function - GUARDS §6); the 4096-page save area refused a 4K panel (8100 pages) silently, before `failed_at` was set - now 16384 pages and a ZLLOG event; a BOUNCE (unplug and re-plug seen as one event, CCS still set) kept a dead slot as live - now detach-then-plug; an unplug the ring never announced - the scan now detaches an owner whose port went dark; an ECM or mass-storage unplug had its slot forgotten under it with `ecm_ready`/`msc_ready` still set, and a stale TX hold outlived an ECM re-init; a controller advertising more than 31 ports would have had them unmanaged - clamped with a WARN. Each has an `xhcitest`/`ggttest` pin. One gate note from landing it: `verify-efi` went red once in the full chain because QEMU itself died under KVM (signal 139) at load 5.2 and the TCG retry missed the timer-preemption proof; alone at load below 3 it is green (25 ok) - the receipt records the flake as load, not code, and the load rule in CLAUDE.md stands. Known limit, written into the probe: the driver speaks no hubs, so a keyboard that comes back behind one (where QEMU puts a fifth device on four USB2 root ports) is not seen; the probe re-plugs into the same root port, which is what a cable does. **Both closed 2026-09-08:** `vmm_walk_present` walks the live tables (1 GiB, 2 MiB and 4 KiB leaves; `pagingtest`, 8 cases against a hand-built table) and `xhci_find` refuses a BAR the identity map cannot reach - `xbar_high`, the same diagnosed state the 32-bit lane reports - instead of faulting on the first register read; and the single ECM TX buffer is held from doorbell to completion, a timed-out send keeps it held and later sends are refused (`ecm_n_full`) until the late completion arrives through `ecm_event` (`xhcitest`, 6 assertions).
- 32-bit lane has no double-fault stack; 64-bit APs have no TSS. **APs closed 2026-09-06:** `gdt64.c` builds one TSS per slot (8, held equal to smp.c's `SMP_SLOTS` by `gdt64test`, which reads the number out of `smp.c`), each with its own 16 KiB IST1 stack, and `smp_ap_main` loads OUR GDT and its slot's TSS before going live - an AP whose `str` does not read back its selector parks instead of joining the band count, so `verify-64`/`verify-efi` (`-smp 2`) witness the ltr. The #DF stack classifier in `idt.c` now resolves the faulting core's stack through `str`. APs keep RSP0 = 0: only the BSP runs ring 3. **32-bit closed 2026-09-08:** vector 8 is a task gate (`idt[8]` type 0x85 → GDT 0x30) into a second TSS with its own 16 KiB stack in `gdt.c`; `fault_df_task32` reads the interrupted context back out of the main TSS and records it. The recorder had a "32-bit has no IST" shortcut that let a 32-bit #DF commit with no emergency stack - now both lanes must present one (`crashtest`, 4 new pins). `crashdftest` on that lane loads a one-byte data segment (GDT 0x38) into SS so its first push is #SS and the delivery faults again - paging is off there, so "RSP on an unmapped page" cannot be the trigger. `verify-crash.py --route bios32 --fault double-fault` is the QEMU witness and is in the landing gate. **TCG follow-up 2026-09-09:** the one-byte segment trigger reached UD2 under software CPU emulation. The replacement forces general protection against a not-present handler gate, producing a real double fault through the same emergency task. Fresh KVM and TCG runs pass with the unchanged vector/checksum/stack/halt oracle; see [the current process evidence](process-spawn-wait-2026-09-08.md).
