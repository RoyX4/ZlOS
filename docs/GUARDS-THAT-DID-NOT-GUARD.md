...
Written 2026-08-19; §6 added 2026-09-04. Five checks in this tree (twenty-two with
...
## 6. Seven more, from the 2026-09-04 whole-tree sweep (two added 2026-09-06, one 2026-09-08)
...
| `kernel/tests/host/zllog_e2e_test.py` (5 cases, ASan, torn writes and identity refusal against the shipping `zllog.c`) and `kernel/tests/host/dpll_test.c` (the only DPLL write harness) | nothing - `gen-test-inventory.py` enumerates executable `.sh` files, so a `.py` gate is invisible to it, and a `.c` with no `build.sh` line has no compiled target to classify. Both written 2026-08-30. **2026-09-06:** the Python passed the first time anyone ran it (5/5, 16 s); the C had not linked since `intel.c` grew `pci_bar_hi`/`console_init_fb`. Now `zllog-e2e.sh` (gate, refuses without `sgdisk` instead of unittest's green SkipTest) and a `dpll_test` build line (manual-hardware). | `python3 kernel/tests/host/zllog_e2e_test.py`; `gcc ... dpll_test.c intel.c` → undefined reference |
| `gputest` (the compositor's GGTT host test) | that `intel_ggtt_map_range` works for the compositor's back-to-back back/scan mappings. It stubs `intel_ggtt_map_range` itself (`gputest.c:53`), and `ggttest` links `intel.c` alone - so when the 2026-09-08 GGTT save/restore made the real function refuse any second range while one was held, every compositor caller (`gpuring.c:495,561,563,654`, `gpucursor.c:171`) was broken and both tests stayed green. Found by a fresh adversarial reviewer the same day; the hold moved to a bring-up-only `intel_ggtt_map_range_saved`, and `ggttest` now maps two plain ranges in a row. | `grep -n intel_ggtt_map_range kernel/tests/host/gputest.c` → a stub |
