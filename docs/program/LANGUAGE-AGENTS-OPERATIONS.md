...
| DEV-012 | Linux ABI compatibility | isolated selected syscall/program workload, not blanket claim |
| DEV-013 | PE/Windows import quarantine | length-first loader/cache for explicit compatibility apps only |
...
| DEV-017 | virtual machine manager | machines, images, devices, snapshots and containment |
| DEV-018 | emulator framework | CPU/device emulation with deterministic fixtures and limits |
| DEV-019 | sandbox/container profiles | namespaces, handles, resources, network and teardown |
| DEV-020 | guest integration | typed shared folder/clipboard/input/display authority |
