...
- VM/emulation/containers/sandbox profiles, virtio guest integration, 9P/shared
  folder and clipboard with explicit authority;
- port availability/build/runtime/security/upgrade evidence, not recipe counts.

Exit: selected reference programs build and run through declared compatibility
contracts; host/target compiler and utility behavior agrees on golden corpora;
guest integration cannot escape granted resources.
