...
Value zl_calln(const char *name, int n, ...)
{
    ...
    if (streq(name, "fs_nlen"))    return zl_num((double)fs_name_len());
    ...
    /* Name the missing boundary. The old generic line made every typo and
     * every source-list omission look identical once the kernel was booted. */
    ser_puts("kernel missing builtin: ");
    ser_puts(name);
    ser_puts("\n");
