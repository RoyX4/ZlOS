...
extern int  xhci_replug_count(void);
extern int  xhci_replug_tries(void);
extern int  xhci_replug_events(void);
extern int  xhci_replug_last_v(void);
extern int  xhci_replug_last_port(void);
extern int  xhci_replug_skips(void);
extern int  xhci_replug_last_try(void);
extern int  xhci_replug_polls(void);
extern int  xhci_owned_flag(void);
extern int  xhci_replug_scans_found(void);
extern int  xhci_port_conn_bitmap(void);
extern unsigned int xhci_replug_pending(void);
extern int  xhci_kbd_port(void);
extern int  xhci_enum_stage(int);
extern int  xhci_enum_cc(int);
...
Value zl_calln(const char *name, int n, ...)
{
    ...
    if (streq(name, "usb_replug")) return zl_num((double)xhci_replug_count());
    if (streq(name, "usb_replug_tries")) return zl_num((double)xhci_replug_tries());
    if (streq(name, "usb_replug_events")) return zl_num((double)xhci_replug_events());
    if (streq(name, "usb_replug_last_v")) return zl_num((double)(unsigned)xhci_replug_last_v());
    if (streq(name, "usb_replug_last_port")) return zl_num((double)xhci_replug_last_port());
    if (streq(name, "usb_replug_skips")) return zl_num((double)xhci_replug_skips());
    if (streq(name, "usb_replug_last_try")) return zl_num((double)xhci_replug_last_try());
    if (streq(name, "usb_replug_polls")) return zl_num((double)xhci_replug_polls());
    if (streq(name, "usb_owned_flag")) return zl_num((double)xhci_owned_flag());
    if (streq(name, "usb_ticks")) return zl_num((double)idt_ticks());
    if (streq(name, "usb_replug_scans")) return zl_num((double)xhci_replug_scans_found());
    if (streq(name, "usb_conn_bits")) return zl_num((double)(unsigned)xhci_port_conn_bitmap());
    if (streq(name, "usb_replug_pending")) return zl_num((double)xhci_replug_pending());
    if (streq(name, "usb_kbd_port")) return zl_num((double)xhci_kbd_port());
    if (streq(name, "usb_enum_stage")) return zl_num((double)xhci_enum_stage((int)a[0].num));
    if (streq(name, "usb_enum_cc"))    return zl_num((double)xhci_enum_cc((int)a[0].num));
